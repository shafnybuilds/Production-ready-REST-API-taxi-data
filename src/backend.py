...
def read_parquet_file(year: int, month: int) -> Optional[pd.DataFrame]:
    """Reads a parquet file from the cache directory.
    ...
    Returns:
        Optional[pd.DataFrame]: The dataframe if the file exists, otherwise None.
    """
    file_path = os.path.join(CACHE_DIR, f"{year}_{month}.parquet")
...
def download_parquet_file(year: int, month: int):
    ...
    Args:
        year (int): The year to download the file for
        month (int): The month to download the file for
    """
...
def read_parquet_file(year: int, month: int) -> Optional[pd.DataFrame]:
    """Read the paquet file

    Args:
        year (int): the year to read the file for
        month (int): the month to read the file for

    Returns:
        Optional[pd.DataFrame]: A pandas DF with the data, or None if the the file couldn't read
    """
    ...
    try:
        df = pd.read_parquet(
            f"{CACHE_DIR}/yellow_tripdata_{year}-{month:02d}.paquet", engine="pyarrow"
        )
    except Exception as e:
        logger.info(f"Failed to read the fils {e}")
        return None

    # i'm gonna filter the df to only columns i need
    df = df[
        [
